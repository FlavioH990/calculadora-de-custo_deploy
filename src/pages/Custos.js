...
function Custos() {
    ...
    useEffect(() => {
        const fetchAllProducts = async () => {
            try {
                const response = await fetch('http://localhost:5000/produtos-cadastrados');
                if (!response.ok) {
    ...
    useEffect(() => {
        ...
        const fetchProductDetails = async () => {
            try {
                setLoading(true);
                const response = await fetch(`http://localhost:5000/produtos-cadastrados/${selectedProduct.ID_Produto}`);
                if (!response.ok) {
