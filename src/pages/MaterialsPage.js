...
export function MaterialsProvider({ children }) {
    ...
    const loadMaterials = async () => {
        if (!user) return;
        try {
            const response = await fetch("http://localhost:5000/materias-primas");
...
export default function MaterialsPage() {
    ...
    const confirmSave = async () => {
        ...
        try {
            const promessas = [];
            promessas.push(fetch(`http://localhost:5000/mapear-atributos`, {
                method: "POST",
            ...
            promessas.push(fetch(`http://localhost:5000/materias-primas/${materialId}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/materias-primas/${materialToDelete.id}`, { method: "DELETE" });
