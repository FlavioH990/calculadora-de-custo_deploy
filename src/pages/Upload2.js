...
function ImportarManual() {
  ...
  useEffect(() => {
      const fetchTodasSugestoes = async () => {
          try {
              const urls = [
                  "http://localhost:5000/materias-primas",
                  "http://localhost:5000/sugestoes/emissores_cnpj",
                  "http://localhost:5000/sugestoes/codigos_produto"
              ];
...
const confirmSave = async () => {
    ...
    try {
        const response = await fetch('http://localhost:5000/adicionar-manual', {
            method: 'POST',
